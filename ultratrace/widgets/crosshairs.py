...
class Crosshairs:
    def __init__(self, zframe, x, y, color, transform=True):
        ...
        self.hline = self.zframe.canvas.create_line(
            ...
            self.x + self.len,
            self.y,
            fill=self.unselectedColor,
            width=self.unselectedWidth,
        )
    ...
    def getDistance(self, click):
        ...
        return (
            math.sqrt(dx**2 + dy**2) if self.isVisible else float("inf")
        )  # invisible points infinitely far away

    def select(self):
        """select this Crosshairs"""
        if self.isVisible:
            self.zframe.canvas.itemconfig(
                self.hline, fill=self.selectedColor, width=self.selectedWidth
            )
            self.zframe.canvas.itemconfig(
                self.vline, fill=self.selectedColor, width=self.selectedWidth
            )
    ...
    def unselect(self):
        ...
        if self.isVisible:
            self.zframe.canvas.itemconfig(
                self.hline, fill=self.unselectedColor, width=self.unselectedWidth
            )
            self.zframe.canvas.itemconfig(
                self.vline, fill=self.unselectedColor, width=self.unselectedWidth
            )
            self.isSelected = False
    ...
    def dragTo(self, click):
        ...
        if self.isVisible:
            ...
            self.trueX, self.trueY = self.transformCoordsToTrue(self.x, self.y)
            self.len = self.transformLength(CROSSHAIR_SELECT_RADIUS)
            self.zframe.canvas.coords(
                self.hline, self.x - self.len, self.y, self.x + self.len, self.y
            )
            self.zframe.canvas.coords(
                self.vline, self.x, self.y - self.len, self.x, self.y + self.len
            )
