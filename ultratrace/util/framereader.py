...
class DicomReader(FrameReader):
    def getFrameTimes(self):
        ...
        try:
            ...
            return [
                float(int.from_bytes(headers[i : i + 4], byteorder="little")) / 1000000 - offset
                for i in range(0, len(headers), 32)
            ]
        except:
...
class DicomScanLineReader(DicomReader):
    ...
    def __init__(self, filename):
        ...
        offsets = [
            int.from_bytes(data[i * 4 + 8 : i * 4 + 12], byteorder="little")
    ...
    def load(self):
        ...
        for f in range(frames):
            printProgressBar(
                ...
                frames,
                prefix="Processing:",
                suffix=("complete (%d of %d)" % (f + 1, frames)),
            )
...
class ULTScanLineReader(FrameReader):
    ...
    def __init__(self, data, metadata):
        ...
        f.close()
        self.loaded = True
        self.radspace = np.linspace(
            self.ZeroOffset, self.ZeroOffset + self.PixPerVector, num=self.PixPerVector
        )
        total = self.Angle * self.NumVectors
        self.thetaspace = np.linspace(
            (math.pi + total) / 2, (math.pi - total) / 2, num=self.NumVectors
        )
    ...
    def getFrame(self, framenum):
        ...
        data = np.ndarray(
            shape=(self.NumVectors, self.PixPerVector), buffer=byt, dtype="uint8"
        ).swapaxes(0, 1)
        fig = plt.figure()
        ...
        px = ax.transData.transform(
            [[0, 0], [math.pi / 2, r], [self.thetaspace[0], r], [self.thetaspace[-1], r]]
        )
