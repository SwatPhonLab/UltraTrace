...
class TextGrid(Module):
    ...
    def makeFrameWidget(self):
        ...
        txtbox = Spinbox(
            sbframe, textvariable=self.frame_shift, width=7, from_=-10000000, to=10000000
        )
        ...
        window = frames_label.create_window(
            self.label_width * 0.3, self.canvas_height / 3, anchor="nw", window=sbframe
        )
        ...
        sbframe.bind(
            "<Configure>", lambda e: frames_label.configure(scrollregion=frames_label.bbox("all"))
        )

        self.TkWidgets.append(
            {"name": self.frameTierName, "frames": self.frames_canvas, "frames-label": frames_label}
        )
    ...
    def makeTierWidgets(self, tier):
        ...
        widgets = {
            ...
            "canvas-label": Canvas(
                self.frame, width=self.label_width, height=self.canvas_height, highlightthickness=0
            ),
                    ...
                    while i < len(tier) and tier[i].minTime <= self.end:
                        ...
                        if self.selectedItem:
                            if self.selectedItem[0] != self.app.Spectrogram.canvas:
                                # old_selected_tags = self.selectedItem[0].gettags(self.selectedItem[1])
                                if (
                                    minTimetag in old_selected_tags
                                    and maxTimetag in old_selected_tags
                                    and canvas == self.selectedItem[0]
                                ):
    ...
    def update(self):
        ...
        if self.selectedItem:
            if (
                self.selectedItem[0] in self.tier_pairs.keys()
                or self.selectedItem[0] in self.tier_pairs.values()
            ):
                if "frame" + str(self.app.frame) not in self.selectedItem[0].gettags(
                    self.selectedItem[1]
    ...
    def grid(self, event=None):
        ...
        for t in range(len(self.TkWidgets)):
            ...
            if "canvas" in tierWidgets:
                ...
                tierWidgets["canvas-label"].grid(
                    row=t, column=0, sticky="w", pady=self.app.pady / 2
                )
