...
class Playback(Module):
    ...
    def __init__(self, app):
        ...
        if AUDIO_LIBS_INSTALLED:
            ...
            self.playBtn = Button(
                self.frame, text="⏯", command=self.playpauseAV, state="disabled", takefocus=0
            )  # NOTE: not currently appearing
    ...
    def playpauseAV(self, event=None):
        ...
        if (
            self.started == False or self.currentInterval != self.app.TextGrid.selectedItem
        ):  # if we haven't started playing or we're in a new interval
    ...
    def readyAudio(self, start, end):
        ...
        self.stream = self.p.open(
            format=self.p.get_format_from_width(self.seg.sample_width),
    ...
    def readyVideo(self):
        ...
        self.dicomframeQ = queue.Queue()
        self.dicomframeQ.put(
            self.pngs[0]
        )  # put now, because audio callback puts frames when audio segments end
        # for i in range(len(self.pngs)):
        #   self.dicomframeQ.put(self.pngs[i])
    ...
    def callback(self, in_data, frame_count, time_info, status):
        ...
        # self.chunkcount+=1
        data = b"".join(
            [self.seg.get_frame(i) for i in range(self.audioframe, self.audioframe + frame_count)]
        )
        ...
        if self.app.Dicom.isLoaded():
            ...
            # go to next frame
            if (
                self.dicomframe_timer % self.flen != self.dicomframe_timer
                and self.dicomframe_num < len(self.pngs)
            ):
    ...
    def playVideoWithAudio(self):
        ...
        try:
            pic = self.dicomframeQ.get(timeout=0.5)
        ...
        if (
            not self.stoprequest.is_set() or not self.dicomframeQ.empty()
    ...
    def playVideoNoAudio(self):
        ...
        canvas.update()
        if (
            not self.dicomframeQ.empty() and self.stoprequest.is_set() == False
        ):  # should this if be at the top?
            self.playVideoNoAudio()
