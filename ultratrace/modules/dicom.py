...
class Dicom(Module):
    ...
    def reset(self):
        ...
        self.methodMenu = OptionMenu(
            self.frame, self.method, "---", *options, command=self.chooseMethod
        )
        self.methodMenu.grid(row=0)
        self.loadBtn = Button(
            self.frame, text="Load frames", command=self.load, takefocus=0, state="disabled"
        )
