...
class Spectrogram(Module):
    def __init__(self, app):
        ...
        self.axis_frame.grid(
            row=0, column=0, sticky="e", pady=(self.app.pady * 2, self.app.pady / 2)
        )
        ...
        # buttons
        default_btn = Button(
            self.spinwin, text="Standards", command=self.restoreDefaults, takefocus=0
        )
        apply_btn = Button(
            self.spinwin, text="Apply", command=self.drawSpectrogram, takefocus=0, width=6
        )
    ...
    def drawSpectrogram(self, event=None):
        ...
        if self.app.Audio.current:
            ...
            spec = sound_clip.to_spectrogram(
                window_length=wl, time_step=self.ts, maximum_frequency=self.spec_freq_max.get()
            )
            ...
            img = img.resize(
                (int(self.canvas_width * (duration / screen_duration)), self.canvas_height)
            )
        ...
        if self.app.TextGrid.selectedItem:
            ...
            if widg in self.app.TextGrid.tier_pairs:  # if widg is label
                ...
                for i in itvl_canvas.find_withtag("line"):
                    ...
                    self.canvas.create_line(
                        loc, 0, loc, self.canvas_height, tags="line", fill="blue"
                    )
                ...
                if itm - 1 in widg.find_all():
                    ...
                    self.canvas.create_line(
                        l_loc, 0, l_loc, self.canvas_height, tags="line", fill="blue"
                    )
                if itm + 1 in widg.find_all():
                    r_loc = widg.coords(itm + 1)[0]
                    self.canvas.create_line(
                        r_loc, 0, r_loc, self.canvas_height, tags="line", fill="blue"
                    )
            ...
            if self.app.TextGrid.firstFrame <= self.app.frame <= self.app.TextGrid.lastFrame:
                xcoord = self.app.TextGrid.frames_canvas.coords(
                    self.app.TextGrid.highlighted_frame
                )[0]
                self.canvas.create_line(
                    xcoord, 0, xcoord, self.canvas_height, tags="line", fill="red"
                )
    ...
    def xToTime(self, x):
        ...
        return (
            x * float(self.app.TextGrid.end - self.app.TextGrid.start) / self.canvas_width
        ) + float(self.app.TextGrid.start)

    def timeToX(self, time):
        """converts from a time to the x coordinate on a canvas representing that time"""
        return (
            self.canvas_width
            * (time - float(self.app.TextGrid.start))
            / float(self.app.TextGrid.end - self.app.TextGrid.start)
        )
